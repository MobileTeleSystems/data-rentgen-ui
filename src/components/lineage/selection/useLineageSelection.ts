...
const useLineageSelection = () => {
    ...
    const setSelection = useCallback(
        (nodesToSelect: Set<string>, edgesToSelect: Set<string>) => {
    ...
    const getAllRelations = useCallback(
        (nodeIds: string[]): { nodes: Set<string>; edges: Set<string> } => {
            const result = {
            ...
            const upstreams = getAllConnection(
                nodeIds,
                edgesByTarget,
                "source",
            );
            const downstreams = getAllConnection(
                nodeIds,
                edgesBySource,
                "target",
            );
